...
def run_simulation(csv_path, seq_len=60, grace_period=20, override_timeout=10, empty_timeout=30, light_timeout=10, ldr_threshold=500):
    ...
    for i in range(seq_len, len(df)):
       ...
       # --- AYDINLATMA KARAR MEKANİZMASI ---
        if current_motion == 0:
            ...
            if user_was_present:
                if continuous_time_empty < 5:
                    # Çıkıştan sonraki ilk 4 dakika (Tolerans süresi)
                    if current_ldr_in < 400:
                        plot_light_command.append("Açık")
                    else:
                        plot_light_command.append("-")
                        
                elif continuous_time_empty == 5:
                    # TAM 5. DAKİKA: Karar anı
                    if current_ldr_in < 400:
                        light_auto_off = True  # Kapatma kararı verildi ve kilitlendi
                        plot_light_command.append("Kapat (Unutulan Işık)")
                    else:
                        plot_light_command.append("-")
                        
                else:
                    # 5 dakikadan sonrası (Örn: 2 saat sonra)
                    # Sadece 5. dakikada kapatıldıysa bu durumu grafikte sürdür
                    # 5. dakikada kapatılmadıysa (güneş doğsa bile) tepki verme
                    if light_auto_off:
                        plot_light_command.append("Kapat (Unutulan Işık)")
                    else:
                        plot_light_command.append("-")
            ...
            # Odada biri varsa sayacı sıfırla, durumu yenile
            continuous_time_empty = 0
            user_was_present = True
            light_auto_off = False  # Biri girince kilidi kaldır
    ...
    # Yeni mantığa uygun olarak "Unutulan Işık" komutunu arıyoruz
    is_light_off_forgotten = np.array([cmd == "Kapat (Unutulan Işık)" for cmd in plot_light_command])

    # Grafikte bu anları belirgin bir kırmızı/pembe tonuyla çizdiriyoruz
    ax_light.fill_between(plot_times, 0, 1, where=is_light_off_forgotten, color='red', alpha=0.25, 
                         transform=ax_light.get_xaxis_transform(), label='KAPAT (Unutulan Işık - 5dk Kuralı)')
...
if __name__ == "__main__":
    ...
    run_simulation('current_status_02_sali.csv', grace_period=20, override_timeout=25, empty_timeout=30, light_timeout=10, ldr_threshold=400)