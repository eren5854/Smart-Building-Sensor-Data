...
def run_simulation(csv_path, seq_len=60, grace_period=20, override_timeout=10, empty_timeout=30, light_timeout=10, ldr_threshold=500):
    ...
    print("Günlük veri dakika dakika taranıyor (Time-lapse)...")
    ...
    for i in range(seq_len, len(df)):
        ...
        # Gün Işığından yararlanma: Dışarısı aydınlıksa (LDR düşükse) ışığı kapat.
    ...
    gs = gridspec.GridSpec(3, 2, width_ratios=[4, 1.2], height_ratios=[2, 2, 2], wspace=0.1, hspace=0.35)
    
    ax_plot = fig.add_subplot(gs[0, 0]) 
    ax_dur = fig.add_subplot(gs[1, 0], sharex=ax_plot) 
    ax_light = fig.add_subplot(gs[2, 0], sharex=ax_plot) 
    ax_text = fig.add_subplot(gs[:, 1]) 

    # --- ÜST GRAFİK: ÇIKIŞ İHTİMALİ VE KLİMA KONTROLÜ ---
    ...
    ax_plot.axhline(y=0.5, color='gray', linestyle='--', alpha=0.5, label='Karar Eşiği (%45)')
    ...
    ax_plot.set_title('Akıllı Bina Simülasyonu: 1) Ofisten Çıkış İhtimali ve Otonom Klima', fontsize=12, fontweight='bold', loc='left')
    ax_plot.set_ylabel('Çıkış İhtimali (0.0 - 1.0)', fontsize=10)
    ...
    # --- ORTA GRAFİK: DIŞARIDA KALMA SÜRESİ KATEGORİSİ ---
    ...
    ax_dur.set_title('2) Odadan Uzak Kalma Süresi Tahmini (Sınıflandırma)', fontsize=12, fontweight='bold', loc='left')
    ax_dur.set_ylabel('Süre Kategorisi', fontsize=10)
    ax_dur.set_yticks([0, 1, 2])
    ax_dur.set_yticklabels(["Kısa Mola\n(0-15dk)", "Uzun Mola\n(15-60dk)", "Paydos\n(>60dk)"])
    ax_dur.grid(True, linestyle=':', alpha=0.7)
    ax_dur.legend(loc='upper left', fontsize=9)
    plt.setp(ax_dur.get_xticklabels(), visible=False)

    # --- ALT GRAFİK: LDR VE AYDINLATMA (Y EKSENİ TERS ÇEVRİLDİ) ---
    ...
    # Boş oda kapanmaları (Siyah Gölge)
    ...
    # Gün ışığı kapanmaları (Sarı Gölge)
    ...
    ax_light.set_title('3) Otonom Aydınlatma: Yüksek Değer = Karanlık / Düşük Değer = Aydınlık', fontsize=12, fontweight='bold', loc='left')
    ax_light.set_xlabel('Gün İçindeki Zaman (Saat:Dakika)', fontsize=10)
    
    # LDR DEĞERİ DÜŞÜKSE AYDINLIK MANTIĞI İÇİN Y EKSENİNİ TERS ÇEVİRİYORUZ
    ax_light.invert_yaxis()
    ax_light.set_ylabel('Işık Şiddeti (LDR)\n↑ Daha Aydınlık', fontsize=10)
    
    ax_light.grid(True, linestyle=':', alpha=0.7)
    ax_light.legend(loc='upper left', fontsize=9)

    time_fmt = mdates.DateFormatter('%H:%M')
    ax_light.xaxis.set_major_formatter(time_fmt)
    fig.autofmt_xdate(rotation=45) 

    # --- 5. SAĞ PANEL: ÖZET METİN VE TABLO ---
    ax_text.axis('off') 
    
    stats_text = (
        "SIMULASYON OZET RAPORU\n"
        "-------------------------------\n"
        f"Gecerli Tahmin: {total_valid_preds}\n"
        f"Klima Oto Kapanma: {empty_timeout} dk (Bos)\n"
        f"Isik Oto Kapanma: {light_timeout} dk (Bos)\n"
        f"Gun Isigi Esigi: < {ldr_threshold} LDR\n\n"
        f"Cikis Tahmini Dogrulugu: %{acc_time:.2f}\n"
        f"Sure Kategorisi Dogrulugu: %{acc_dur:.2f}\n"
    )
    
    ax_text.text(0.05, 0.98, stats_text, transform=ax_text.transAxes, fontsize=10, weight='bold',
                 verticalalignment='top', bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))

    if not df_results.empty:
        table_sample = df_results.iloc[::max(1, len(df_results)//15)].head(15).copy() 
    ...
    plt.savefig('simulation_result_with_table.png', dpi=300, bbox_inches='tight')
    print("Grafik 'simulation_result_with_table.png' olarak kaydedildi!")
    plt.show()

if __name__ == "__main__":
    # ldr_threshold değeri dış mekanın aydınlık sayılacağı maksimum ldr değeridir.
    run_simulation('formated_current_status_23_sali.csv', grace_period=20, override_timeout=25, empty_timeout=30, light_timeout=10, ldr_threshold=400)